...
typedef struct TupleTableSlotBlock
{
    TupleTableSlot *tuples[2]; //TupleTable
    int cursor=0; /* pointer to the next tuple to be used*/
    /*TupleTableSlot */
}TupleTableSlotBlock;

/*
void InitTupleTableBlock(EState *estate, TupleTableSlotBlock *ttsBlock){

}
*/

bool TupBlockIsNull(TupleTableSlotBlock *ttsBlock){
    if (ttsBlock->cursor >= 2) {
		ttsBlock->cursor = 0;
		return true;
	}
    else return false;
}
void ExecProcNodeBlock(PlanState *outerPlan, TupleTableSlotBlock *ttsBlock){
	for(int i=0; i<2;i++){
		ttsBlock->tuples[i] = ExecProcNode(outerPlan);
	}
	ttsBlock->cursor = 0;
}
TupleTableSlot *GetTupleFromBlock(TupleTableSlotBlock *ttsBlock){
	TupleTableSlot *newtuple;
	ExecCopySlot(newtuple,ttsBlock->tuples[ttsBlock->cursor]);
	return newtuple;
}
...
ExecBlockNestLoop(PlanState *pstate)
{
	...
	ListCell   *lc;
    TupleTableSlotBlock *TTSBlock;
	
	EState *estate;
	//estate = node->js.ps.state;

	//InitTupleTableBlock(estate,TTSBlock);
	...
	for (;;)
	{
		...
		if (node->nl_NeedNewOuter)
		{
            ...
            ExecProcNodeBlock(outerPlan,TTSBlock);

			outerTupleSlot0 = TTSBlock->tuples[0];

			/*
			 * if there are no more outer tuples, then the join is complete..
			 */
			if (TupIsNull(outerTupleSlot0)
			{
