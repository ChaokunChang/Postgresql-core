...
#define TRANSFER(x, y) ((int)toupper(VARDATA((x))[(y)]))
...
/*user's functions: Leevnshstein Distance functions*/
...
int levenshtein_function(const char* str1, int row,const char* str2,int column){
	int Distance[150][150];

    //initialize the Distance Matrix
    for(int i=0; i<row+1;i++) Distance[i][0] = i;//column 0;
    for(int j=1; j<column+1;j++) Distance[0][j] = j;//row 0

	for(int j=1; j<=column; j++){
        for(int i=1; i<=row; i++){
			if( toupper(str1[i-1]) == toupper(str2[j-1]) ) Distance[i][j] = Distance[i-1][j-1];
			else Distance[i][j] = min_dis(Distance[i-1][j]+1,Distance[i][j-1]+1,Distance[i-1][j-1] + 1);
        }
    }
...
Datum levenshtein_distance(PG_FUNCTION_ARGS) { 
	text    *str01 = PG_GETARG_TEXT_PP(0); 
	text	*str02 = PG_GETARG_TEXT_PP(1);
	
	const char *str1 = VARDATA_ANY(str01);
	const char *str2 = VARDATA_ANY(str02); 

    int row,column;
    row = VARSIZE_ANY_EXHDR(str01);//string vertical
    column = VARSIZE_ANY_EXHDR(str02);//string herotical

	PG_RETURN_INT32(levenshtein_function(str1,row,str2,column));
}

int levenshtein_function1(const char* str1, int row,const char* str2,int column){
	int Distance[150][150];
...
Datum levenshtein_distance1(PG_FUNCTION_ARGS) { 
    //elog(LOG, "levenshtein_distance has been executed.");
	text    *str01 = PG_GETARG_TEXT_PP(0); 
	text	*str02 = PG_GETARG_TEXT_PP(1);
	...
	PG_RETURN_INT32(levenshtein_function1(str1,row,str2,column));
}

Datum levenshtein_distance2(PG_FUNCTION_ARGS) { 
    //elog(LOG, "levenshtein_distance has been executed.");
	text    *str01 = (text *)PG_GETARG_TEXT_P(0); 
	text	*str02 = (text *)PG_GETARG_TEXT_P(1);

    int row,column;
    row = VARSIZE(str01)-VARHDRSZ;//string vertical
    column = VARSIZE(str02)-VARHDRSZ;//string herotical
    static int Distance[150][150] = {0};

    //initialize the Distance Matrix
    for(int i=0; i<=row;i++) Distance[i][0] = i;//column 0;
    for(int j=1; j<=column;j++) Distance[0][j] = j;//row 0
    
    for(int j=1; j<=column; j++){
        for(int i=1; i<=row; i++){
			if( (int)toupper(VARDATA(str01)[i-1]) == (int)toupper(VARDATA(str02)[j-1]) ) Distance[i][j] = Distance[i-1][j-1];
			else Distance[i][j] = min_dis(Distance[i-1][j],Distance[i][j-1],Distance[i-1][j-1])+1;
        }
    }
	PG_RETURN_INT32(Distance[row][column]);
}

Datum levenshtein_distance3(PG_FUNCTION_ARGS) { 
	const char *str1 = PG_GETARG_CSTRING(0);
	const char *str2 = PG_GETARG_CSTRING(1); 
    int row,column;
    
    row = VARSIZE_ANY_EXHDR(str1);//string vertical
    column = VARSIZE_ANY_EXHDR(str2);//string herotical
    int Distance[150][150] = {0};

    //initialize the Distance Matrix
    for(int i=0; i<row+1;i++) Distance[i][0] = i;//column 0;
    for(int j=0; j<column+1;j++) Distance[0][j] = j;//row 0
...
Datum jaccard_index(PG_FUNCTION_ARGS)
{
    ...
    int clean[300];//store the positions to be cleaned
    int len1 = length_t(str1), len2 = length_t(str2);
    int cnt = 0, cnt_intersec = 0;
    int i,pos;
    if (len1 && len2)
    {
//store the first string
        pos = TRANSFER(str1, 0);
        flag[pos] = 1;
        clean[cnt++] = pos;//store the first position;not need check
        for (i = 1; i < len1; i++)
        {
            pos = SHIFT(pos, TRANSFER(str1, i));//pos%128 = str1[i-1]
            if (!flag[pos])
            {
            flag[pos] = 1;
            clean[cnt++] = pos;
        ...
        pos = SHIFT(pos,0);//the last position
        if(!flag[pos]) {
            flag[pos] = 1;
            clean[cnt++] = pos;
        }
//check the second string
        ...
        cnt_intersec++;
        ans = (float4)cnt_intersec/(float4)cnt;
        for (i = 0; i < cnt; i++)
        flag[clean[i]] = 0;
    }
