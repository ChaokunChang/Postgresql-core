 ...
 *		satisfies the qualification clause.//cck（限定从句）
 *
...
ExecNestLoop(PlanState *pstate)
{
	...
	for (;;)
	{
		...
		if (node->nl_NeedNewOuter)
		{
			...
			outerTupleSlot = ExecProcNode(outerPlan);//cck 使用函数从外关系计划中读取一个tupleslot，
		...
		if (TupIsNull(innerTupleSlot))
		{
				 ...
				 node->js.jointype == JOIN_ANTI))
			{
				...
				{// CCK 不是很明白otherqual的作用，似乎是标记Outer已经没有未匹配的元组了，下面就直接返回了
					...
					return ExecProject(node->js.ps.ps_ProjInfo);//CCK 按照ProjInfo的Projection信息返回结果
				}
				else
